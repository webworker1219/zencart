...
*/
  ...
  $get_shipping_list = $new_Shipping_4px->get_list_shipping_estimates($_POST['zone_country_id'],$total_weight,$_SESSION['currency']);
  $shipping_4px      = array();
  foreach($get_shipping_list as $key=>$value){
	...
	if($total_weight>=$get_max_shipping_weight or empty($value['shipping_price'])){//计算不出运费或运输价格为0 就不输出
	  continue;
	}
